...
const AdminPayments = () => {
  ...
  return (
    <div className="space-y-6 max-w-[1400px] font-['Inter']">
      <div className="flex items-center justify-between"><h1 className="text-xl font-bold text-[#1A1A1E]">Payments</h1><button className="flex items-center gap-2 px-3 py-2 border border-[#E5E7EB] rounded-lg text-xs font-medium text-[#6B7280] hover:bg-[#F9FAFB]"><Download className="w-3.5 h-3.5" /> Export</button></div>
      <div className="grid grid-cols-3 gap-3">
        {[{l:"Pending",v:`AED ${fmt(pending.reduce((s,p)=>s+Number(p.amount),0))}`,n:pending.length,c:"#F59E0B"},{l:"Processing",v:`AED ${fmt(processing.reduce((s,p)=>s+Number(p.amount),0))}`,n:processing.length,c:"#3B82F6"},{l:"Paid",v:`AED ${fmt(paid.reduce((s,p)=>s+Number(p.amount),0))}`,n:paid.length,c:"#22C55E"}].map(s=>(<div key={s.l} className="bg-white rounded-xl border border-[#E5E7EB] p-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)]"><p className="text-lg font-bold text-[#1A1A1E]">{s.v}</p><p className="text-[11px] font-medium mt-0.5" style={{color:s.c}}>{s.l} ({s.n})</p></div>))}
      </div>
      <div className="flex flex-wrap items-center gap-3">
        ...
        {["all","pending","processing","paid"].map(s=>(<button key={s} onClick={()=>{setStatusFilter(s);setPage(1)}} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${statusFilter===s?"bg-[#1A1A1E] text-white":"bg-white border border-[#E5E7EB] text-[#6B7280]"}`}>{s==="all"?"All":s.charAt(0).toUpperCase()+s.slice(1)}</button>))}
      </div>
