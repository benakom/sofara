...
const AdminAmbassadorDetail = () => {
  ...
  if (loading || !profile) return <div className="flex justify-center py-20"><div className="w-6 h-6 rounded-full border-2 border-[#154B3B] border-t-transparent animate-spin" /></div>;
  ...
  const stagePill = (s: string) => { const c: Record<string,string> = { nouveau:"bg-[#6B7280]/10 text-[#6B7280]", contacté:"bg-[#F59E0B]/10 text-[#F59E0B]", qualifié:"bg-[#22C55E]/10 text-[#22C55E]", négociation:"bg-[#8B5CF6]/10 text-[#8B5CF6]", closing:"bg-[#D2F34C]/20 text-[#154B3B]", perdu:"bg-[#EF4444]/10 text-[#EF4444]" }; return <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${c[s]||c.nouveau}`}>{s}</span>; };
    ...
    <div className="space-y-6 max-w-[1400px] font-['Poppins']">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div><div className="flex items-center gap-3"><h1 className="text-xl font-bold text-[#154B3B]">{profile.full_name || "—"}</h1>{statusPill(profile.status)}</div><p className="text-xs text-[#9CA3AF] mt-0.5">Joined {new Date(profile.created_at).toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" })}</p></div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 bg-[#D2F34C] text-black rounded-lg text-xs font-bold hover:bg-[#BDE040]"><Save className="w-3.5 h-3.5" /> Save</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        <div className="lg:col-span-3 space-y-5">
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <h2 className="text-sm font-bold text-[#154B3B] mb-4">Profile Information</h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[#F9FAFB] rounded-lg p-3 flex items-center gap-2"><span className="text-[10px] font-semibold text-[#9CA3AF]">Referral Code:</span><span className="text-xs font-mono font-bold text-[#154B3B]">{profile.referral_code||"—"}</span>{profile.referral_code&&<button onClick={()=>{navigator.clipboard.writeText(profile.referral_code);toast({title:"Copied!"})}}><Copy className="w-3 h-3 text-[#9CA3AF]" /></button>}</div>
              <div className="bg-[#F9FAFB] rounded-lg p-3"><span className="text-[10px] font-semibold text-[#9CA3AF]">Type: </span><span className="text-xs font-bold text-[#154B3B]">{profile.profile_type||"—"}</span></div>
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <h2 className="text-sm font-bold text-[#154B3B] mb-4">Performance Metrics</h2>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
              {[{l:"Total Leads",v:leads.length},{l:"Qualified",v:`${qualifiedLeads.length} (${qualifiedRate}%)`},{l:"Deals Closed",v:closedDeals},{l:"Commission",v:`AED ${fmt(totalComm)}`}].map(m=>(<div key={m.l} className="bg-[#F9FAFB] rounded-xl p-3"><p className="text-lg font-bold text-[#154B3B]">{m.v}</p><p className="text-[10px] text-[#9CA3AF]">{m.l}</p></div>))}
            </div>
            {leadsChart.length>0&&<ResponsiveContainer width="100%" height={160}><LineChart data={leadsChart}><CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" /><XAxis dataKey="month" tick={{fontSize:10,fill:"#9CA3AF"}} axisLine={false} tickLine={false} /><YAxis tick={{fontSize:10,fill:"#9CA3AF"}} axisLine={false} tickLine={false} /><Tooltip contentStyle={{background:"#154B3B",border:"none",borderRadius:8,fontSize:11,color:"#fff"}} /><Line type="monotone" dataKey="count" stroke="#D2F34C" strokeWidth={2} dot={{fill:"#D2F34C",r:3}} /></LineChart></ResponsiveContainer>}
          </div>
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <h2 className="text-sm font-bold text-[#154B3B] mb-4">Commission History</h2>
            {commissions.length>0?<div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-[#E5E7EB]"><th className="text-left py-2 text-[10px] font-semibold text-[#9CA3AF] uppercase">Date</th><th className="text-left py-2 text-[10px] font-semibold text-[#9CA3AF] uppercase">Deal</th><th className="text-left py-2 text-[10px] font-semibold text-[#9CA3AF] uppercase">Amount</th><th className="text-left py-2 text-[10px] font-semibold text-[#9CA3AF] uppercase">Status</th></tr></thead><tbody>{commissions.map(c=>(<tr key={c.id} className="border-b border-[#F5F5F7]"><td className="py-2 text-xs text-[#6B7280]">{new Date(c.created_at).toLocaleDateString("en-GB",{day:"2-digit",month:"short"})}</td><td className="py-2 text-xs font-medium text-[#154B3B]">{c.deal_name}</td><td className="py-2 text-xs font-bold text-[#154B3B]">AED {fmt(c.amount)}</td><td className="py-2">{statusPill(c.status)}</td></tr>))}</tbody></table></div>:<p className="text-xs text-[#9CA3AF] text-center py-6">No commissions yet</p>}
          </div>
        </div>
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <h2 className="text-sm font-bold text-[#154B3B] mb-3">Lead Activity</h2>
            <div className="space-y-0 max-h-[300px] overflow-y-auto">{leads.slice(0,15).map((l,i)=>(<div key={l.id} className={`flex items-center gap-3 py-2.5 ${i<leads.length-1?"border-b border-[#F5F5F7]":""}`}><div className="flex-1 min-w-0"><p className="text-xs font-medium text-[#154B3B] truncate">{l.first_name} {l.last_name}</p><p className="text-[10px] text-[#9CA3AF]">{new Date(l.created_at).toLocaleDateString("en-GB",{day:"2-digit",month:"short"})}</p></div>{stagePill(l.stage||"nouveau")}</div>))}{leads.length===0&&<p className="text-xs text-[#9CA3AF] text-center py-6">No leads yet</p>}</div>
          </div>
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <h2 className="text-sm font-bold text-[#154B3B] mb-3">Referrals ({referrals.length})</h2>
            <div className="space-y-0">{referrals.map((r,i)=>(<div key={r.id} className={`flex items-center gap-3 py-2.5 ${i<referrals.length-1?"border-b border-[#F5F5F7]":""}`}><div className="w-7 h-7 rounded-full bg-[#154B3B]/10 flex items-center justify-center text-[9px] font-bold text-[#154B3B]">{(r.full_name||"?")[0]}</div><div className="flex-1 min-w-0"><p className="text-xs font-medium text-[#154B3B] truncate">{r.full_name||"—"}</p></div>{statusPill(r.status)}</div>))}{referrals.length===0&&<p className="text-xs text-[#9CA3AF] text-center py-6">No referrals</p>}</div>
          </div>
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
