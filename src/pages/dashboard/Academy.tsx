...
const categories = [
  { labelFr: "Tout", labelEn: "All", icon: GraduationCap, id: "all" },
  { labelFr: "Immobilier Dubai", labelEn: "Dubai Real Estate", icon: TrendingUp, id: "dubai" },
  { labelFr: "Techniques de vente", labelEn: "Sales Techniques", icon: Users, id: "sales" },
  { labelFr: "Conformité", labelEn: "Compliance", icon: CheckCircle, id: "compliance" },
];

const levelConfig: Record<string, { color: string; bg: string; labelFr: string; labelEn: string }> = {
  beginner: { color: "text-emerald-700", bg: "bg-emerald-100", labelFr: "Débutant", labelEn: "Beginner" },
  intermediate: { color: "text-amber-700", bg: "bg-amber-100", labelFr: "Intermédiaire", labelEn: "Intermediate" },
  advanced: { color: "text-rose-700", bg: "bg-rose-100", labelFr: "Avancé", labelEn: "Advanced" },
};
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            {ytId ? (
              ...
              <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-black mb-5 shadow-lg">
                ...
                <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                  ...
                  <p className="absolute bottom-4 text-white text-sm font-medium">{lang === "fr" ? "Vidéo bientôt disponible" : "Video coming soon"}</p>
                </div>
              </div>
            ) : (
              <div className="w-full aspect-video rounded-2xl bg-[hsl(var(--dash-card))] border border-[hsl(var(--dash-border))] flex items-center justify-center mb-5">
                <p className="dash-muted-text text-base sm:text-sm">{lang === "fr" ? "Vidéo bientôt disponible" : "Video coming soon"}</p>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              {extras.isNew && (
                <span className="text-xs font-bold px-3 py-1 rounded-full bg-emerald-500 text-white">{lang === "fr" ? "Nouveau" : "New"}</span>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-4">
              ...
              <span className="text-sm dash-muted-text flex items-center gap-1"><Users className="w-3.5 h-3.5" /> {extras.students.toLocaleString()} {lang === "fr" ? "inscrits" : "enrolled"}</span>
              <span className="text-sm dash-muted-text flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {selectedCourse.duration}</span>
              <span className="text-sm dash-muted-text flex items-center gap-1"><BookOpen className="w-3.5 h-3.5" /> {selectedCourse.lessons_count} {lang === "fr" ? "leçons" : "lessons"}</span>
            </div>

            <div className="bg-[hsl(var(--dash-card))] border border-[hsl(var(--dash-border))] rounded-xl p-5 mb-5">
              <h2 className="text-lg font-display font-semibold dash-text mb-3">{lang === "fr" ? "À propos de ce cours" : "About this course"}</h2>
            ...
            {/* What you'll learn */}
            <div className="bg-[hsl(var(--dash-card))] border border-[hsl(var(--dash-border))] rounded-xl p-5 mb-5">
              <h2 className="text-lg font-display font-semibold dash-text mb-3">{lang === "fr" ? "Ce que vous apprendrez" : "What you'll learn"}</h2>
                ...
                {Array.from({ length: Math.min(selectedCourse.lessons_count, 6) }).map((_, i) => (
                  <div key={i} className="flex items-start gap-2">
                    ...
                    <span className="text-sm dash-muted-text">
                      {lang === "fr" ? `Module ${i + 1} — Leçon interactive` : `Module ${i + 1} — Interactive lesson`}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[hsl(var(--dash-card))] border border-[hsl(var(--dash-border))] rounded-xl p-5 sticky top-20">
              ...
              <div className="space-y-2.5 text-sm">
                <div className="flex items-center gap-2 dash-muted-text"><Clock className="w-4 h-4" /> {selectedCourse.duration} {lang === "fr" ? "de contenu" : "of content"}</div>
                <div className="flex items-center gap-2 dash-muted-text"><BookOpen className="w-4 h-4" /> {selectedCourse.lessons_count} {lang === "fr" ? "leçons" : "lessons"}</div>
                <div className="flex items-center gap-2 dash-muted-text"><Zap className="w-4 h-4 text-amber-500" /> +{selectedCourse.xp} XP {lang === "fr" ? "à gagner" : "to earn"}</div>
                <div className="flex items-center gap-2 dash-muted-text"><Award className="w-4 h-4" /> {lang === "fr" ? "Certificat de complétion" : "Completion certificate"}</div>
              </div>
            </div>
            ...
            {relatedCourses.length > 0 && (
              <div className="bg-[hsl(var(--dash-card))] border border-[hsl(var(--dash-border))] rounded-xl p-4">
                <h3 className="text-sm font-display font-semibold dash-text mb-3">{lang === "fr" ? "Cours similaires" : "Related courses"}</h3>
                  ...
                  {relatedCourses.map(rc => (
                    <button key={rc.id} onClick={() => { setSelectedCourse(rc); window.scrollTo(0, 0); }}
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-medium dash-text line-clamp-2 leading-tight">{lang === "fr" ? rc.title_fr : rc.title_en}</p>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div className="mb-5">
        ...
        <p className="dash-muted-text text-base sm:text-sm mt-1">
          {lang === "fr" ? "Formez-vous et devenez un expert de l'immobilier à Dubai" : "Train yourself and become a Dubai real estate expert"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          { labelFr: "Cours", labelEn: "Courses", value: courses.length, icon: BookOpen, accent: "bg-blue-100 text-blue-600" },
          { labelFr: "Heures +", labelEn: "Hours +", value: `${totalHours}h`, icon: Clock, accent: "bg-violet-100 text-violet-600" },
          { labelFr: "Leçons", labelEn: "Lessons", value: totalLessons, icon: GraduationCap, accent: "bg-emerald-100 text-emerald-600" },
          { labelFr: "Votre XP", labelEn: "Your XP", value: `${userXP}/${nextLevelXP}`, icon: Zap, accent: "bg-amber-100 text-amber-600" },
        ].map((s, i) => (
            ...
            <div className="absolute inset-0 p-5 sm:p-8 flex flex-col justify-end">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium px-3 py-1 rounded-full bg-white/20 text-white backdrop-blur-sm">
                  {(levelConfig[featuredCourse.level] || levelConfig.beginner)[lang === "fr" ? "labelFr" : "labelEn"]}
                </span>
              </div>
              <h2 className="text-xl sm:text-2xl lg:text-3xl font-display font-bold text-white mb-2 max-w-xl">
                {lang === "fr" ? featuredCourse.title_fr : featuredCourse.title_en}
              </h2>
              <p className="text-sm text-white/80 max-w-lg line-clamp-2 mb-3 hidden sm:block">
                {lang === "fr" ? featuredCourse.description_fr : featuredCourse.description_en}
              </p>
              <div className="flex items-center gap-4 text-sm text-white/70">
                <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {featuredCourse.duration}</span>
                <span className="flex items-center gap-1"><BookOpen className="w-3.5 h-3.5" /> {featuredCourse.lessons_count} {lang === "fr" ? "leçons" : "lessons"}</span>
                <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" /> 1,247 {lang === "fr" ? "inscrits" : "enrolled"}</span>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-display font-semibold dash-text">
          {activeCategory === "all"
            ...
            : categories.find(c => c.id === activeCategory)?.[lang === "fr" ? "labelFr" : "labelEn"]
          }
      ...
      {loading ? (
        ...
        <div className="text-center py-16 dash-muted-text">
          ...
          <p className="text-base">{lang === "fr" ? "Aucun cours trouvé" : "No courses found"}</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="relative h-40 sm:h-44 overflow-hidden bg-[hsl(var(--muted))]">
                  ...
                  <div className="absolute top-2.5 left-2.5 flex gap-1.5">
                    ...
                    {extras.isNew && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-500 text-white shadow-sm">{lang === "fr" ? "Nouveau" : "New"}</span>
                    )}
                  </div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-[hsl(var(--dash-border)/.5)]">
                    <div className="flex items-center gap-3 text-xs dash-muted-text">
                      <span className="flex items-center gap-1"><BookOpen className="w-3.5 h-3.5" /> {course.lessons_count} {lang === "fr" ? "leçons" : "lessons"}</span>
                    </div>
      ...
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="dash-card rounded-xl p-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="text-sm font-semibold dash-text">{lang === "fr" ? "Niveau" : "Level"} {userLevel}</p>
        ...
        <div className="dash-card rounded-xl p-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-sm font-semibold dash-text">{lang === "fr" ? "Cours terminés" : "Completed"}</p>
        ...
        <div className="dash-card rounded-xl p-4">
          ...
          <div className="flex gap-2">
            {[
              { icon: "🏅", label: lang === "fr" ? "1er cours" : "1st course" },
              { icon: "🔥", label: lang === "fr" ? "Série 7j" : "7-day streak" },
              { icon: "⭐", label: "500 XP" },
              { icon: "🏆", label: "Top 10" },
              { icon: "💎", label: "Expert" },
            ].map((badge, i) => (
