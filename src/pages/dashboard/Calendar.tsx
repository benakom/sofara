  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[hsl(var(--dash-muted-fg))]">
            {lang === "ar" ? "Organisez vos rendez-vous et relances" : "Manage your appointments and follow-ups"}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-lg border border-[hsl(var(--dash-border))] overflow-hidden">
            ...
            >
              <CalendarDays className="w-3.5 h-3.5" /> {lang === "ar" ? "Mois" : "Month"}
            </button>
            <button
          ...
          >
            ...
            <span className="hidden sm:inline">{lang === "ar" ? "Nouvel événement" : "New event"}</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: lang === "ar" ? "Aujourd'hui" : "Today", value: todayStats.total, sub: lang === "ar" ? "événements" : "events" },
          { label: lang === "ar" ? "Appels prévus" : "Planned calls", value: todayStats.calls, sub: lang === "ar" ? "aujourd'hui" : "today" },
                  ...
                  return (
                    ...
                    >
                      <span className={`inline-flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 rounded-full text-xs sm:text-sm font-medium
                        ...
                        ${isSelected && !today ? "ring-2 ring-[hsl(var(--dash-accent))] text-[hsl(var(--dash-accent-ink))]" : "text-[hsl(var(--dash-fg))]"}
                      `}>
                        {format(day, "d")}
                      </span>
            ...
            <div className="divide-y divide-[hsl(var(--dash-border))]">
              <div className="px-4 sm:px-6 py-4">
                <h2 className="text-base font-semibold text-[hsl(var(--dash-fg))] font-display">
                  {lang === "ar" ? "Événements à venir" : "Upcoming events"}
                </h2>
              </div>
              {upcomingEvents.length === 0 ? (
                <div className="px-6 py-12 text-center text-[hsl(var(--dash-muted-fg))] text-sm">
                  {lang === "ar" ? "Aucun événement prévu" : "No upcoming events"}
                </div>
              ) : (
        ...
        <div className="space-y-4">
            ...
            <div className="px-4 py-3 border-b border-[hsl(var(--dash-border))] flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-[hsl(var(--dash-muted-fg))]">
                  {selectedEvents.length} {lang === "ar" ? "événement(s)" : "event(s)"}
                </p>
              </div>
              <button
              ...
              {selectedEvents.length === 0 ? (
                <div className="px-4 py-8 text-center">
                  ...
                  <p className="text-xs text-[hsl(var(--dash-muted-fg))]">
                    {lang === "ar" ? "Aucun événement" : "No events"}
                  </p>
                  <button
                    onClick={() => openNewEvent(selectedDate)}
                    className="mt-2 text-xs text-[hsl(var(--dash-accent-ink))] font-medium hover:underline"
                  >
                    {lang === "ar" ? "+ Ajouter" : "+ Add"}
                  </button>
                </div>
          ...
          <div className="dash-card rounded-xl px-4 py-3">
            <p className="text-xs font-semibold text-[hsl(var(--dash-fg))] mb-2">
              {lang === "ar" ? "Types d'événements" : "Event types"}
            </p>
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                <h3 className="text-base font-semibold text-slate-900 font-display">
                  {editingEvent
                    ? (lang === "ar" ? "Modifier l'événement" : "Edit event")
                    : (lang === "ar" ? "Nouvel événement" : "New event")}
                </h3>
              ...
              <div className="px-5 py-4 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                {/* Title */}
                <div>
                  <label className="text-xs font-medium text-slate-600 mb-1 block">{lang === "ar" ? "Titre" : "Title"} *</label>
                  <input
                    value={formTitle}
                    onChange={e => setFormTitle(e.target.value)}
                    placeholder={lang === "ar" ? "Ex: Appel avec Ahmed — Off-plan Emaar" : "Ex: Call with Ahmed — Off-plan Emaar"}
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    <label className="text-xs font-medium text-slate-600 mb-1 block">{lang === "ar" ? "Début" : "Start"} *</label>
                    <input type="time" value={formTime} onChange={e => setFormTime(e.target.value)}
                      className="w-full px-2.5 py-2 rounded-lg border border-slate-200 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary)/.3)]"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-slate-600 mb-1 block">{lang === "ar" ? "Fin" : "End"}</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text-xs font-medium text-slate-600 mb-1 block">{lang === "ar" ? "Lieu" : "Location"}</label>
                    <input value={formLocation} onChange={e => setFormLocation(e.target.value)}
                      placeholder={lang === "ar" ? "Ex: Showroom DAMAC" : "Ex: DAMAC Showroom"}
                      className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary)/.3)]"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    placeholder={lang === "ar" ? "Notes supplémentaires..." : "Additional notes..."}
