  ...
  return (
    <button
      ...
      className={`group text-left rounded-2xl border p-5 transition-all duration-200 hover:-translate-y-0.5 ${
        emphasis
          ? "bg-[hsl(var(--dash-accent))] border-transparent text-[hsl(var(--dash-fg))] shadow-[var(--dash-accent-glow)]"
          : "bg-[hsl(var(--dash-card))] border-[hsl(var(--dash-border))] text-[hsl(var(--dash-fg))] hover:border-[hsl(var(--dash-card-ring))]"
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className={`text-[12px] font-medium truncate ${emphasis ? "text-[hsl(var(--dash-fg))]/60" : "text-[hsl(var(--dash-muted-fg))]"}`}>{label}</p>
        ...
        <span className={`shrink-0 w-9 h-9 rounded-xl flex items-center justify-center ${emphasis ? "bg-black/10" : "bg-[hsl(var(--dash-accent)/.12)]"}`}>
          <Icon className={`w-4 h-4 ${emphasis ? "text-[hsl(var(--dash-fg))]" : "text-[hsl(var(--dash-accent-ink))]"}`} />
        </span>
      </div>
      <div className="mt-4 flex items-end justify-between gap-3">
        <div className="min-w-0">
          {hasDelta ? (
            <span className={`inline-flex items-center gap-1 text-[12px] font-semibold ${
              emphasis ? "text-[hsl(var(--dash-fg))]/75" : good === null ? "text-[hsl(var(--dash-muted-fg))]" : good ? "text-[hsl(var(--dash-accent-ink))]" : "text-[hsl(var(--dash-warning))]"
            }`}>
              <DeltaIcon className="w-3.5 h-3.5" />
              {dir === "flat" ? "0%" : `${deltaPct > 0 ? "+" : ""}${deltaPct}%`}
              {deltaLabel && <span className={`font-normal ${emphasis ? "text-[hsl(var(--dash-fg))]/50" : "text-[hsl(var(--dash-muted-fg))]"}`}>{deltaLabel}</span>}
            </span>
          ) : deltaLabel ? (
            <span className={`text-[12px] ${emphasis ? "text-[hsl(var(--dash-fg))]/60" : "text-[hsl(var(--dash-muted-fg))]"}`}>{deltaLabel}</span>
          ) : null}
        </div>
        {trend && trend.length > 1 && (
          <Sparkline
            ...
            accent={emphasis ? "#0d3a2b" : "hsl(78 70% 30%)"}
